...
type ProbableOriginAsset = {
  ...
  downstream_asset_id?: number | string | null;
  relationship_type?: string | null;
  distance_meters?: number | string | null;
  estimated_travel_time_minutes?: number | string | null;
  relationship_confidence?: number | string | null;
  evidence?: unknown;
};
...
type AttributionTopology = {
  ...
  upstream_relationship_count?: number | string;
  probable_origin_assets?: ProbableOriginAsset[];
};
...
function formatAssessmentType(value?: string): string {
  ...
  if (normalized === 'probabilistic_topology_aware') {
    return 'Probabilistic - Topology-Aware';
  }

  if (normalized === 'probabilistic_inferred') {
    return 'Probabilistic - Inferred';
  }
...
function normalizeAssetTypeLabel(value?: string | null): string {
  const raw = String(value ?? '').trim();

  if (!raw) return 'Unknown asset type';
...
function normalizeRelationshipConfidence(value: unknown): number | null {
...
export function SourceAttribution() {
  ...
  const [attributionData, setAttributionData] =
    useState<AttributionResponse>(fallbackAttributionData);
  ...
  useEffect(() => {
    const loadAttribution = async () => {
      try {
        ...
        const probableOriginAssets: ProbableOriginAsset[] = Array.isArray(
          result?.probable_origin_assets,
        )
          ? result.probable_origin_assets
          : Array.isArray(result?.topology?.probable_origin_assets)
            ? result.topology.probable_origin_assets
            : [];
        ...
        setAttributionData({
          ...
          updated_at: result?.updated_at ?? result?.generated_at,
          ...
          primary: {
            ...
            indicator:
              ...
              result?.summary ??
              fallbackAttributionData.primary?.indicator,
          },
          ...
          topology: {
            ...
            max_depth: result?.topology?.max_depth,
            upstream_relationship_count: result?.topology?.upstream_relationship_count,
            probable_origin_assets: probableOriginAssets,
          },

          recommendations: {
            immediate: toStringArray(
              result?.recommendations?.immediate ?? result?.immediate_actions,
              toStringArray(fallbackAttributionData.recommendations?.immediate, []),
            ),
            followUp: toStringArray(
              result?.recommendations?.followUp ?? result?.follow_up_actions,
  ...
  const primaryWhyTags = useMemo(() => {
    ...
    if (topologyUsed) {
      if (source.includes('building')) {
        return [
          'Topology evidence included',
          'Premise-proximal source pattern',
          'Upstream asset path evaluated',
        ];
      }
      ...
      if (source.includes('distribution')) {
        return [
          'Topology evidence included',
          'Shared-line pathway supported',
          'Upstream assets ranked',
        ];
      }

      const hasTravelTimeEvidence =
        Array.isArray(attributionData.probable_origin_assets) &&
        attributionData.probable_origin_assets.some(
          (asset) =>
            toNullableFiniteNumber(asset.estimated_travel_time_minutes) !== null,
        );

      return [
        'Topology evidence included',
        'Upstream source pathway supported',
        hasTravelTimeEvidence
          ? 'Hydraulic travel-time context used'
          : 'Relationship confidence evaluated',
      ];
    }
  ...
  const probableOriginAssets = Array.isArray(attributionData.probable_origin_assets)
    ? attributionData.probable_origin_assets
    : [];

  const topologyUsed = Boolean(attributionData.topology?.used);

  const upstreamRelationshipCount = toFiniteNumber(
    attributionData.topology?.upstream_relationship_count,
    0,
  );
    ...
    <div className="fx-app-shell min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <section className="mb-8 p-6 rounded-lg bg-zinc-900/50 border border-zinc-800">
          ...
          {topologyUsed && probableOriginAssets.length > 0 ? (
            <div className="space-y-4">
              {probableOriginAssets.map((asset, index) => {
                const relationshipConfidence = normalizeRelationshipConfidence(
                  asset.relationship_confidence,
                );
                const rankingScore = toNullableFiniteNumber(asset.score);
                const depth = toFiniteNumber(asset.depth, 0);
                const evidence = toStringArray(asset.evidence, []);
                        ...
                        <div>
                          ...
                          <div className="space-y-1.5">
                            {evidence.map((item, evidenceIndex) => (
                              <div
                                key={`${asset.asset_id ?? index}-evidence-${evidenceIndex}`}
                        ...
                        <div className="flex items-center gap-4 mb-3">
                          <div
                            className={`p-2 rounded ${
                              isTop ? 'bg-amber-500/20' : 'bg-zinc-700/40'
                            }`}
                          >
                          ...
                          <div className="text-right">
                            <p
                              className={`text-2xl font-light ${normalizeSourceProbabilityColor(
                                probability,
                              )}`}
                            >
                              {probability}%
